...
确认订单页（选地址 → 选优惠券 → 金额明细 → 备注 → 提交）
        ↓
创建订单（校验库存，占用优惠券，待付款 30 分钟超时自动取消）
        ↓
模拟支付 / 微信支付
        ↓
...
├── sitemap.json
├── utils/
│   ├── mock.js                      # 商品/分类/订单 mock 数据与操作（含库存、超时取消、搜索/热搜词、优惠券占用回退）
...
- 购物车存于 storage key `cartList`，收货地址存于 `addressList`，订单存于 `orderList`，收藏存于 `favoriteList`（商品 id 数组），搜索历史存于 `searchHistory`，用户信息存于 `userInfo`，优惠券存于 `couponList`，评价存于 `reviewList`，设置存于 `settingsList`
